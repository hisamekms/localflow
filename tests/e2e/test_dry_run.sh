...
DR_ADD="$(run_lf --dry-run --output json add --title "test task" --priority p1 --tag rust)"
...
DR_ADD_TEXT="$(run_lf --dry-run --output text add --title "test task")"
...
ADD_OUT="$(run_lf --output json add --title "Complete Me")"
TASK_ID="$(echo "$ADD_OUT" | jq -r '.id')"
run_lf edit "$TASK_ID" --status todo >/dev/null
run_lf next >/dev/null

DR_COMPLETE="$(run_lf --dry-run --output json complete "$TASK_ID")"
...
ADD_OUT2="$(run_lf --output json add --title "Draft Task")"
TASK2_ID="$(echo "$ADD_OUT2" | jq -r '.id')"
# Task is in draft state, cannot complete directly
if run_lf --dry-run complete "$TASK2_ID" 2>/dev/null; then
...
ADD_OUT3="$(run_lf --output json add --title "Cancel Me")"
TASK3_ID="$(echo "$ADD_OUT3" | jq -r '.id')"
run_lf edit "$TASK3_ID" --status todo >/dev/null

DR_CANCEL="$(run_lf --dry-run --output json cancel "$TASK3_ID" --reason "not needed")"
...
# Verify status didn't change
GET_OUT3="$(run_lf --output json get "$TASK3_ID")"
...
DR_NEXT="$(run_lf --dry-run --output json next)"
...
GET_OUT3B="$(run_lf --output json get "$TASK3_ID")"
assert_json_field "$GET_OUT3B" '.status' "todo" "status unchanged after dry-run next"

# 8. dry-run edit
echo "[8] dry-run edit"
DR_EDIT="$(run_lf --dry-run --output json edit "$TASK3_ID" --title "New Title" --priority p0)"
...
GET_OUT3C="$(run_lf --output json get "$TASK3_ID")"
assert_json_field "$GET_OUT3C" '.title' "Cancel Me" "title unchanged after dry-run edit"

# 9. dry-run deps add
echo "[9] dry-run deps add"
DR_DEPS="$(run_lf --dry-run --output json deps add "$TASK3_ID" --on "$TASK2_ID")"
...
DR_DEPS_RM="$(run_lf --dry-run --output json deps remove "$TASK3_ID" --on "$TASK2_ID")"
...
DR_DEPS_SET="$(run_lf --dry-run --output json deps set "$TASK3_ID" --on "$TASK2_ID")"
