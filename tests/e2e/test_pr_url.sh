...
CLEAR_OUT="$(run_lf edit "$TASK_ID" --clear-pr-url)"
assert_json_field "$CLEAR_OUT" '.pr_url' "null" "pr_url cleared to null"

echo "[5] text output shows pr_url"
run_lf edit "$TASK_ID" --pr-url "https://github.com/org/repo/pull/99" > /dev/null
...
run_lf edit "$TASK_ID" --clear-pr-url > /dev/null
TEXT_OUT2="$(run_lf --output text get "$TASK_ID")"
...
run_lf edit "$TASK_ID" --pr-url "https://github.com/org/repo/pull/1" > /dev/null
...
DRY_OUT="$(run_lf --dry-run edit "$TASK_ID" --pr-url "https://example.com/pr/2")"
