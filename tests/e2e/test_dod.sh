...
ADD_OUT="$(run_lf --output json add --title "DoD Task" --definition-of-done "item1" --definition-of-done "item2")"
...
# Check first item
OUT="$(run_lf --output json dod check "$TASK_ID" 1)"
CHECKED="$(echo "$OUT" | jq -c '[.definition_of_done[].checked]')"
assert_eq '[true,false]' "$CHECKED" "after check 1"

# Check second item
OUT="$(run_lf --output json dod check "$TASK_ID" 2)"
CHECKED="$(echo "$OUT" | jq -c '[.definition_of_done[].checked]')"
assert_eq '[true,true]' "$CHECKED" "after check 2"

# Uncheck first item
OUT="$(run_lf --output json dod uncheck "$TASK_ID" 1)"
CHECKED="$(echo "$OUT" | jq -c '[.definition_of_done[].checked]')"
assert_eq '[false,true]' "$CHECKED" "after uncheck 1"

# 2. Index out of range
echo "[2] Index out of range"
assert_exit_code 1 run_lf --output json dod check "$TASK_ID" 0
assert_exit_code 1 run_lf --output json dod check "$TASK_ID" 3
...
run_lf --output json dod check "$TASK_ID" 1 >/dev/null

OUT="$(run_lf --output json complete "$TASK_ID")"
