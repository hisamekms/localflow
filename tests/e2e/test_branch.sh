...
OUT="$(run_lf --output json task add --title "task with branch" --branch "feature/my-branch")"
...
OUT="$(run_lf --output json task add --title "no branch task")"
assert_json_field "$OUT" '.branch' "null" "branch is null when not set"

# 6. ${task_id} template in add
echo "[6] \${task_id} template in add"
OUT="$(run_lf --output json task add --title "template task" --branch 'task-${task_id}-feature')"
...
OUT="$(run_lf --output json task edit "$TASK_ID" --branch 'work/${task_id}')"
...
OUT="$(run_lf --output json task add --title "plain task")"
