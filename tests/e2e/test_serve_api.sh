...
wait_for "API server ready" 10 "curl -sf $BASE/stats >/dev/null"
...
echo ""
echo "=== Create task ==="
TASK1=$(api_json -X POST "$BASE/tasks" -d '{"title":"Task One","description":"First task"}')
...
echo ""
echo "=== Get task ==="
GOT=$(api_get "$BASE/tasks/$TASK1_ID")
...
TASK2=$(api_json -X POST "$BASE/tasks" -d '{"title":"Task Two","priority":"P1","tags":["backend"],"definition_of_done":["Write tests","Deploy"]}')
...
echo ""
echo "=== List tasks ==="
LIST=$(api_get "$BASE/tasks")
assert_eq "2" "$(echo "$LIST" | jq 'length')" "list returns 2 tasks"

echo ""
echo "=== Edit task ==="
EDITED=$(api_json -X PUT "$BASE/tasks/$TASK1_ID" -d '{"title":"Task One Updated","add_tags":["frontend"]}')
...
echo ""
echo "=== Ready task ==="
READY=$(api_json -X POST "$BASE/tasks/$TASK1_ID/ready" -d '{}')
assert_json_field "$READY" '.status' "todo" "ready transitions to todo"

echo ""
echo "=== Ready task2 ==="
api_json -X POST "$BASE/tasks/$TASK2_ID/ready" -d '{}' >/dev/null

echo ""
echo "=== Start task ==="
STARTED=$(api_json -X POST "$BASE/tasks/$TASK1_ID/start" -d '{"session_id":"test-session"}')
...
DEP_REMOVED=$(api_json -X DELETE "$BASE/tasks/$TASK4_ID/deps/$TASK3_ID")
assert_eq "0" "$(echo "$DEP_REMOVED" | jq '.dependencies | length')" "dependency removed"

echo ""
echo "=== Cancel task ==="
api_json -X POST "$BASE/tasks/$TASK3_ID/ready" -d '{}' >/dev/null
CANCELED=$(api_json -X POST "$BASE/tasks/$TASK3_ID/cancel" -d '{"reason":"no longer needed"}')
...
TASK5=$(api_json -X POST "$BASE/tasks" -d '{"title":"Next Candidate","priority":"P0"}')
TASK5_ID=$(echo "$TASK5" | jq -r '.id')
api_json -X POST "$BASE/tasks/$TASK5_ID/ready" -d '{}' >/dev/null

NEXT=$(api_json -X POST "$BASE/tasks/next" -d '{}')
...
api_json -X POST "$BASE/tasks/$TASK5_ID/complete" -d '{}' >/dev/null 2>&1 || true
api_json -X POST "$BASE/tasks/$TASK4_ID/ready" -d '{}' >/dev/null 2>&1 || true
api_json -X POST "$BASE/tasks/$TASK4_ID/cancel" -d '{"reason":"cleanup"}' >/dev/null 2>&1 || true
api_json -X POST "$BASE/tasks/$TASK7_ID/cancel" -d '{"reason":"cleanup"}' >/dev/null 2>&1 || true
api_json -X POST "$BASE/tasks/$TASK8_ID/cancel" -d '{"reason":"cleanup"}' >/dev/null 2>&1 || true
STATUS_NEXT_EMPTY=$(api_status -X POST "$BASE/tasks/next" -d '{}')
