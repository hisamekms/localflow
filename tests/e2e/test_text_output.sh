...
ADD_TEXT="$(run_lf --output text add --title "Text Test Task")"
...
LIST_TEXT="$(run_lf --output text list)"
...
NEXT_TEXT="$(run_lf --output text next)"
...
COMP_TEXT="$(run_lf --output text complete "$TASK_ID")"
...
CANC_ADD="$(run_lf --output json add --title "Cancel Text Test")"
CANC_ID="$(echo "$CANC_ADD" | jq -r '.id')"
CANC_TEXT="$(run_lf --output text cancel "$CANC_ID" --reason "test reason")"
