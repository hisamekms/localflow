...
フックは CLI / server コマンドがタスク状態を変更した際に自動実行されるシェルコマンドです。デーモン不要でインラインで発火し、デフォルトでは CLI をブロックしません。各フックは名前付きエントリで、その挙動（sync/async、pre/post、失敗時 abort/warn、必須環境変数、選定結果フィルタ）は定義自体に記述します。スキーマ全体は [設定リファレンス → Hooks](CONFIGURATION.ja.md#hooks) を参照してください。
...
CLI / server の各 runtime でのアクション集合は固定です: `task_add` / `task_ready` / `task_start` / `task_complete` / `task_cancel` / `task_select`。
...
`senko hooks test <event_name> [task_id]` で単一フックを実際のタスクまたはサンプルタスクのペイロードで同期実行できます。有効なイベント名: `task_add`, `task_ready`, `task_start`, `task_complete`, `task_cancel`, `task_select`。
