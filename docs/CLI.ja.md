...
## フック – タスク / contract 状態変更時の自動アクション

フックは CLI / server コマンドがタスクまたは contract の状態を変更した際に自動実行されるシェルコマンドです。デーモン不要でインラインで発火し、デフォルトでは CLI をブロックしません。各フックは名前付きエントリで、その挙動（sync/async、pre/post、失敗時 abort/warn、必須環境変数、選定結果フィルタ）は定義自体に記述します。スキーマ全体は [設定リファレンス → Hooks](CONFIGURATION.ja.md#hooks) を参照してください。

### 設定

フックは起動バイナリに対応する runtime 別のセクション配下に定義します:

- `[cli.<action>.hooks.<name>]` — ローカル CLI（`senko serve` / `senko serve --proxy` 以外）
- `[server.remote.<action>.hooks.<name>]` — 直接サーバー（`senko serve`）
- `[server.relay.<action>.hooks.<name>]` — リレーサーバー（`senko serve --proxy`）
- `[workflow.<stage>.hooks.<name>]` — Claude Code skill が駆動するワークフローステージ

CLI / server の各 runtime でのアクション集合は固定です。タスクアクション: `task_add` / `task_ready` / `task_start` / `task_complete` / `task_cancel` / `task_select`。Contract アクション: `contract_add` / `contract_edit` / `contract_delete` / `contract_dod_check` / `contract_dod_uncheck` / `contract_note_add`。Contract アクションに `sync`+`pre` + `on_failure = "abort"` を設定した hook は、タスクの状態遷移と同様に `senko contract <verb>` コマンド自体を中止させます。
...
同一アクションに複数のフックを登録できます（名前で区別）。各エントリは `enabled`, `when`, `mode`, `on_failure`, `env_vars` をサポートします:
...
| アクション | トリガー |
|-----------|---------|
| `task_add` | `senko task add` で新しいタスクを作成 |
| `task_ready` | `senko task ready` でタスクを draft から todo に遷移 |
| `task_start` | `senko task start` または `senko task next` でタスクを開始 |
| `task_complete` | `senko task complete` でタスクを完了 |
| `task_cancel` | `senko task cancel` でタスクをキャンセル |
| `task_select` | `senko task next` がタスクを選定／該当タスクを見つけられなかったとき。`on_result = "selected"` / `"none"` / `"any"`（デフォルト `"any"`）で絞り込み。`on_result = "none"` は旧 `on_no_eligible_task` イベントを置き換えます。 |
| `contract_add` | `senko contract add` で contract を作成 |
| `contract_edit` | `senko contract edit` で contract を更新 |
| `contract_delete` | `senko contract delete` で contract を削除 |
| `contract_dod_check` | `senko contract dod check` で contract の DoD 項目をチェック |
| `contract_dod_uncheck` | `senko contract dod uncheck` で contract の DoD 項目をアンチェック |
| `contract_note_add` | `senko contract note add` で contract に note を追加 |

フックは **stdin** でイベントペイロード（JSON）を受け取り、`sh -c` で実行されます。Contract アクションでは contract id や DoD 項目の index は stdin の `event.contract` ペイロード内に含まれます（環境変数としては自動注入されません）。利用者独自の環境変数は従来どおり hook の `env_vars` で opt-in できます。

### フックのテスト

`senko hooks test <event_name> [task_id]` で単一フックを実際のペイロードまたはサンプルペイロードで同期実行できます。有効なイベント名: `task_add`, `task_ready`, `task_start`, `task_complete`, `task_cancel`, `task_select`, `contract_add`, `contract_edit`, `contract_delete`, `contract_dod_check`, `contract_dod_uncheck`, `contract_note_add`。
...
| フィールド | 型 | 説明 |
|-------|------|-------------|
| `runtime` | string | `"cli"`, `"server.relay"`, または `"server.remote"` |
...
| `event` | string | イベント名（例: `"task_add"`, `"task_complete"`, `"task_select"`） |
...
| `unblocked_tasks` | array \| null | このイベントで新たにブロック解除されたタスク（`task_complete` のみ） |
...
`task_complete` イベントで、タスク完了により他のタスクのブロックが解除された場合に含まれます。
...
`contract_*` イベントは外側のエンベロープ（`runtime`, `backend`, `project`, `user`, `event`）は共通ですが、内側の `task` ペイロードが `contract` ペイロードに置き換わります。`from_status` / `stats` / `ready_count` / `unblocked_tasks` はタスクアグリゲート専用のため、contract イベントには含まれません。

| フィールド | 型 | 説明 |
|-------|------|-------------|
| `event_id` | string | UUID v4 一意識別子 |
| `event` | string | イベント名（`contract_add`, `contract_edit`, `contract_delete`, `contract_dod_check`, `contract_dod_uncheck`, `contract_note_add`） |
| `timestamp` | string | ISO 8601（RFC 3339）タイムスタンプ |
| `contract` | object \| null | contract オブジェクト全体（`senko contract get` と同じスキーマ: `id`, `title`, `description`, `definition_of_done`, `tags`, `notes`, `is_completed` …）。aggregate を再読込できなかった稀な失敗経路のみ `null` になります。 |
...
フック定義は環境変数では設定できません。`.senko/config.toml` の runtime 別セクション（`[cli.<action>.hooks.<name>]`、`[server.remote.<action>.hooks.<name>]`、`[server.relay.<action>.hooks.<name>]`、`[workflow.<stage>.hooks.<name>]`）で定義してください。スキーマ全体は [設定リファレンス → Hooks](CONFIGURATION.ja.md#hooks) を参照。
...
プロジェクトの `.senko` ディレクトリをマウントすることで、`config.toml` 内の `[server.remote.*.hooks.*]` 定義もサーバーに読み込まれます。
