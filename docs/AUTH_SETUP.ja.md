...
export SENKO_CLI_REMOTE_TOKEN="管理者から受け取ったAPIキー"
```
...
CLI ──→ 中継サーバー (senko serve) ──→ リモートサーバー
         [server.relay.url 設定済み]     [認証有効]
```

`server.relay.url` が設定された状態で `senko serve` を実行すると、インスタンスはリレーモードで動作します:

- ローカルでの認証はスキップされます（アップストリームサーバーに委任）
- リレーはクライアントの `Authorization` ヘッダーから Bearer トークンを取得します
- リクエストは以下のいずれかのトークンでリモートサーバーに転送されます:
  - リレー自身の `server.relay.token`（設定されている場合）— 優先
  - クライアントの元のトークン（パススルー）
...
export SENKO_SERVER_RELAY_TOKEN="リモートサーバーで発行されたリレー用APIキー"
```
...
```bash
export SENKO_CLI_REMOTE_URL="http://relay-server:3142"
export SENKO_CLI_REMOTE_TOKEN="クライアント自身のAPIキー"
```
...
| **中継サーバー設定** | `server.relay.url` + `server.relay.token` | `server.relay.url` のみ |
...
| `[server.relay]` | `url` | string | - | アップストリームリモートサーバーURL（リレーモードを有効化） |
| `[server.relay]` | `token` | string | - | アップストリームAPIトークン（転送リクエストに付与） |
...
> **補足**: リレーモードでは、中継サーバーの `[server.relay]` セクションがアップストリームのリモートサーバーを指定します。`server.relay.url` を設定した状態で `senko serve` を実行するとリレーモードが有効になります。`server.relay.token`（設定されている場合）はクライアントのトークンの代わりに転送リクエストに付与されます。
...
| `SENKO_CLI_REMOTE_URL` | `cli.remote.url` | APIサーバーURL |
| `SENKO_CLI_REMOTE_TOKEN` | `cli.remote.token` | APIトークン（クライアント側） |
| `SENKO_SERVER_RELAY_URL` | `server.relay.url` | アップストリームリモートサーバーURL（リレーモード） |
| `SENKO_SERVER_RELAY_TOKEN` | `server.relay.token` | アップストリームAPIトークン（リレーモード） |
